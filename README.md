...
基于 [Astro](https://astro.build) 的双语技术博客（**中文为主**，关键系列提供中英对照），部署于 GitHub Pages（`naphjohn.github.io`）。
...
1. 在 GitHub **新建仓库 `naphjohn.github.io`**（用户页必须此名）。
2. 仓库 **Settings → Pages → Source 选 "GitHub Actions"**。
3. 在本目录执行：
   ```bash
   git remote add origin git@github.com:NaphJohn/naphjohn.github.io.git
   git add .
   git commit -m "init: Astro bilingual blog (Ep1 + Ep5)"
   git branch -M main
   git push -u origin main
   ```
4. 等待 Actions 跑完（约 1–2 分钟），访问 https://naphjohn.github.io

> 若用 HTTPS 而非 SSH，把 remote 换成 `https://github.com/NaphJohn/naphjohn.github.io.git`，推送时按提示登录 GitHub。
